...
class SummonEngine {
  ...
  calculate(reservations, stylists, assistants, menus, lunchOverrides = {}, restOverrides = {}) {
    ...
    const overlappingPairs = this._findOverlappingSlots(reservations, menuMap);
    const requiredSlots = this._getRequiredSlots(
      reservations, menuMap, overlappingPairs, stylists, assistants, stylistRates
    );
    ...
    const { assignments, concurrentAssignments, unfilledSlots, autoSlots } = this._assignAssistants(filteredSlots, assistants);
  ...
  _findOverlappingSlots(reservations, menuMap) {
    ...
    // 各スタイリストの予約同士で重なりをチェック
    byStylist.forEach((stylistReservations) => {
      for (let i = 0; i < stylistReservations.length; i++) {
        for (let j = i + 1; j < stylistReservations.length; j++) {
          const a = stylistReservations[i];
          const b = stylistReservations[j];
          const aStart = this._toTimestamp(a.startTime);
          const aEnd = this._toTimestamp(a.endTime);
          const bStart = this._toTimestamp(b.startTime);
          const bEnd = this._toTimestamp(b.endTime);

          // 時間が重なっているかチェック
          if (aStart < bEnd && bStart < aEnd) {
            overlappingReservationIds.add(a.id);
            overlappingReservationIds.add(b.id);
          }
        }
      }
    });

    return overlappingReservationIds;
  }
   ...
   */
  _getRequiredSlots(reservations, menuMap, overlappingReservationIds, stylists = [], assistants = [], stylistRates = {}) {
    const slots = [];
    const isMinutesMode = reservations.length > 0 && typeof reservations[0].startTime === 'number';
    ...
    reservations.forEach(res => {
      ...
      const isOverlapping = overlappingReservationIds.has(res.id);
      ...
      menu.assistantSlots.forEach((slot, index) => {
        ...
        const slotObj = {
          ...
          skipSummon: nonOverlapEval ? nonOverlapEval.skipSummon : false,
  ...
  _applyManncellFallback(assignments, unfilledSlots, autoSlots, requiredSlots, assistants, reservations, overlappingPairs) {
    ...
    for (const block of overlappingBlocks) {
      ...
      manncells.push({
        ...
        teamSize: block.reservations.length + 1, // スタイリスト1人 ＋ 予約数（最大同時稼働アシスタント数）
        isSuccess: allFilled,
...
const res1 = { id: '1', stylistId: 's1', startTime: '09:00', endTime: '11:30', menus: ['m1'] };
const res2 = { id: '2', stylistId: 's1', startTime: '10:00', endTime: '11:30', menus: ['m1'] };
const stylists = [{ id: 's1', name: 'Stylist1' }];
const assistants = [{ id: 'a1', name: 'A1', skills: ['s1'] }];
const menuMap = new Map([['m1', { id: 'm1', assistantSlots: [{ requiredSkill: 's1', startTime: '09:00', endTime: '09:30' }] }]]);
try { const res = engine.calculate([res1, res2], stylists, assistants, menuMap); console.log('manncells:', res.manncells.length); } catch (e) { console.error(e); }
